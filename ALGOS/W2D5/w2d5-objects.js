...
// 2nd // doesn't take into count the falsy values such as null/undefined
// for (var i = 0; i < pokemon.length; i++) {
// 	if (pokemon[i].types[1]) {
// 		console.log(pokemon[i]);
// 	}
// }
// console.log(pokemon[1].types[1] === true);
...
for (var i = 0; i < pokemon.length; i++) {
	if (pokemon[i].types.length === 1 && pokemon[i].types[0] === "poison") {
		...
		var reversed = "";
		for (var j = thisName.length - 1; j >= 0; j--) {
			reversed = reversed + thisName[j];
		}
